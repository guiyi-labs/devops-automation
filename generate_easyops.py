"""
【弃用声明】本文件是 EasyOps 初始脚手架的一次性生成脚本（1b661e9 之前由它生成骨架）。

- 脚本内嵌的默认密钥 / 密码（如 root123456、easyops-secret-key-2026、admin123）
  仅用于最初生成代码，不代表当前运行行为；
- E1 安全与配置基线（2026-08-14，见 docs/changes/2026-08-14-e1-security-baseline.md）
  已将配置迁移到环境变量，并禁止生产环境使用任何内置默认值；
- 本脚本不再用于部署，保留仅作历史归档，请勿据此配置新环境。
"""
